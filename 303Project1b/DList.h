...
#include "list_const_iterator.h"
	...
	DList() : head(NULL), tail(NULL), num_items(0) { }

	/** Construct a copy of a list. */
	DList(const DList<Item_Type>& other) : head(NULL),
	...
	DList<Item_Type>& operator=(const DList<Item_Type>& other) {
		// Make a copy of the other list.
		DList<Item_Type> temp_copy(other);
	...
	void swap(DList<Item_Type> other){
